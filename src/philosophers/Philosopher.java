 ...
 * recurso compartido {@link DinningRoomPorter} el cual deja pasar a n-1
...
public class Philosopher extends Thread {
    ...
    private final DinningRoomPorter diner;
    // Variables para los elementos gráficos:
    private final JLabel jLabel_F;
    private final JLabel jLabel_T_right;
    private final JLabel jLabel_T_left;
    private final Log log; // Para escribir en el Log
    private final JTextField jTextField_C; // Contador de comidas
    private final int time; // Tiempo de comer
     ...
     * @param time Tiempo que se demora en comer
     */
    public Philosopher(int id, Fork right, Fork left, DinningRoomPorter diner, JLabel jLabel_F,
            ...
            Log log,
            JTextField jTextField_C, int time, int quantity) {
    ...
    /**
     * Método que se ejecuta indefinidamente, una por cada hilo creado en
     * {@link Principal}
     */
    @Override
    public void run() {
    ...
    private void runPhilosopherNoGUI() {
         while (true) {
            try { // try / catch
                ...
                if (!left.takeLeftFork(id, log)) {
                    ...
                    if (ControlGUI.getjTextArea_Log() != null) {
                        log.wirteLog(" El Filósofo " + (id + 1) + " tendrá que soltar el tenedor " + (id + 1) + " por exceso de tiempo y salir a pensar.");
                    }
                    ...
                    if (ControlGUI.getjTextArea_Log() != null) {
                        log.wirteLog(" El Filósofo " + (id + 1) + " está pensando.");
                    }
                    try {
                ...
                ControlGUI.filoCount[id]++;
                System.out.println("El Filósofo " + (id + 1) + " está comiendo.");
                if (ControlGUI.getjTextArea_Log() != null) {
                    log.wirteLog(" El Filósofo " + (id + 1) + " está comiendo.");
                }
                ...
                ex.printStackTrace();
                System.err.println("Se ha producido un error. Descripción: " + ex);
                try {
                    if (ControlGUI.getjTextArea_Log() != null) {
                        log.wirteLog("\n\n Se ha producido un error. Descripción: " + ex + "\n\n");
                    }
        ...
        try {
            if (ControlGUI.getjTextArea_Log() != null) {
                log.wirteLog(" La cena ha terminado para todos: El Filósofo " + (id + 1) + " se ha puesto a pensar.\n\n Pulsar Iniciar para continuar.\n\n");
            }
        } catch (InterruptedException ex) {
    ...
    private void runPhilosopherGUI() {
        while (true) {
            try { // try / catch
                // Obtener el comensal para poder comer:
                diner.takeDiner(id, log);
